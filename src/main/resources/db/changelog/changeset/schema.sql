...
(
    ...
    is_feat_or_stats_ready BOOLEAN                                 NOT NULL,
...
--changeset custom_edits:12

CREATE table IF NOT EXISTS custom_edits
(
    id           BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    character_id bigint                                  not null references characters (id),
    custom_edit  varchar                                 not null,
    modificator  integer                                 not null,
    CONSTRAINT pk_custom_edits PRIMARY KEY (id)
);
...
--changeset proficiencies:15

CREATE table IF NOT EXISTS masteries
(
    id   BIGINT GENERATED BY DEFAULT AS IDENTITY NOT NULL,
    name VARCHAR                                 NOT NULL,
    CONSTRAINT pk_masteries PRIMARY KEY (id)
);

--changeset characters_proficiencies:16

create table if not exists characters_masteries
(
    fk_character bigint not null references characters (id),
    fk_mastery   bigint not null references masteries (id)
);